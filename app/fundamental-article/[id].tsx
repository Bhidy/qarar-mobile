...
export default function FundamentalArticleScreen() {
  const { id } = useLocalSearchParams<{ id: string }>();
  const C = useColors();
  const { language, isRTL } = useTheme();
  const { FUNDAMENTAL_ARTICLES } = useData();
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: C.bg.base }} edges={["top"]}>
      <View style={[styles.header, { borderBottomColor: C.border.subtle }, isRTL && styles.rowRTL]}>
        ...
        <Text style={[styles.headerTitle, { color: C.text.primary }]}>{isAr ? "مقال أساسي" : "Fundamental Article"}</Text>
        <View style={{ width: 36 }} />
      </View>
          ...
          <View style={styles.body}>
            ...
            {blocks.map((b, bi) => (
              <View key={bi} style={{ marginTop: Spacing[4] }}>
                <Text style={[styles.blockLabel, { color: b.muted ? C.text.muted : C.text.primary, fontFamily: ff("800") }, isRTL && styles.right]}>
                  {b.label}
                </Text>
                {b.paras.map((p: string, i: number) => (
                  <Text
                    key={i}
                    style={[
                      b.muted ? styles.disclaimerPara : styles.para,
                      { color: b.muted ? C.text.muted : C.text.secondary, fontFamily: ff("400") },
                      isRTL && styles.right,
                    ]}
                  >
                    {p}
