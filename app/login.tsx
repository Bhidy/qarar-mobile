...
export default function LoginScreen() {
  ...
  const { signInWithPassword, signUp, resetPassword, verifyResetOtp, updatePassword, restoreBiometricSession, signInWithApple, signInWithGoogle, user } = useAuth();
  ...
  /** Shared completion for BOTH social providers — mirrors the password path. */
  ...
  async function handleSocialSignIn(provider: "apple" | "google") {
    ...
    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
    try {
      const r = provider === "apple" ? await signInWithApple() : await signInWithGoogle();
      if (r.canceled) return; // user backed out — not an error
      if (r.error) { setErrors({ general: localizeAuthError(r.error, isAr) }); return; }
      ...
      >
        ...
        >
          ...
          <View style={s.formCard}>
            ...
            {/* Social sign-in now lives BELOW the primary CTA (see SocialRow after it). */}
            ...
            <View style={s.form}>
                ...
                {/* ── Social sign-in — compact icon row BELOW the primary CTA.
                    Two equal icon buttons (Apple official black logo-only +
                    Google "G"); equal size keeps them Guideline-4.8 equivalent.
                    Apple logo-only buttons are officially sanctioned by Apple's
                    HIG (1:1 artwork, instantly identifiable). ── */}
                {showModeSwitcher && (
                  <View style={s.socialWrap}>
                    <View style={[s.socialDivider, { flexDirection: isAr ? "row-reverse" : "row" }]}>
                      <View style={s.socialDividerLine} />
                      <Text style={[s.socialDividerText, { fontFamily: ff("600") }]}>
                        {T("or continue with", "أو المتابعة عبر")}
                      </Text>
                      <View style={s.socialDividerLine} />
                    </View>
                    <View style={[s.socialRow, { flexDirection: isAr ? "row-reverse" : "row" }]}>
                      {Platform.OS === "ios" && (
                        <Pressable
                          onPress={() => handleSocialSignIn("apple")}
                          disabled={busy || socialBusy !== null}
                          accessibilityRole="button"
                          accessibilityLabel={T("Sign in with Apple", "تسجيل الدخول عبر Apple")}
                          style={({ pressed }) => [
                            s.socialIconBtn, s.appleIconBtn,
                            (pressed || socialBusy === "apple") && { opacity: 0.8 },
                            (busy || socialBusy !== null) && socialBusy !== "apple" && { opacity: 0.45 },
                          ]}
                        >
                          {socialBusy === "apple" ? (
                      ...
                      <Pressable
                        ...
                        disabled={busy || socialBusy !== null}
                        accessibilityRole="button"
                        accessibilityLabel={T("Sign in with Google", "تسجيل الدخول عبر Google")}
                        style={({ pressed }) => [
                          s.socialIconBtn, s.googleIconBtn,
                          (pressed || socialBusy === "google") && { opacity: 0.8 },
                          (busy || socialBusy !== null) && socialBusy !== "google" && { opacity: 0.45 },
                        ]}
                      >
                        {socialBusy === "google" ? (
...
const s = StyleSheet.create({
  ...
  // ── Social sign-in — compact icon row below the CTA ──
  socialWrap: { marginTop: Spacing[5], gap: Spacing[4] },
  socialRow: { alignItems: "center", justifyContent: "center", gap: 14 },
  socialIconBtn: {
    width: 74,
    height: 52,
    borderRadius: 16,
  ...
  appleIconBtn: { backgroundColor: "#000000" },
  googleIconBtn: {
    backgroundColor: BRAND.bg,
    borderWidth: 1,
  ...
  socialDivider: { alignItems: "center", gap: 12 },
  socialDividerLine: { flex: 1, height: 1, backgroundColor: BRAND.border },
  socialDividerText: { color: BRAND.muted, fontSize: 12 },
